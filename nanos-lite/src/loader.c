...
static uintptr_t loader(PCB *pcb, const char *filename) {
  ...
  for (int i = 0; i < buf_Eheader.e_phnum; i++) {
    fs_lseek(fd, buf_Pheader[i].p_offset, SEEK_SET);
      ...
      #ifdef DEBUG
        Log("[Loader] Generating pages from 0x%08x to 0x%08x", buf_Pheader[i].p_vaddr & ADDRMASK, (buf_Pheader[i].p_vaddr + buf_Pheader[i].p_memsz - 1) & ADDRMASK);
      #endif
...
#ifdef DEBUG
  Log("[uload] Initiating user stack at 0x%08x", mem_top);
#endif
...
#ifdef DEBUG
  Log("[uload] User stack started at 0x%08x", mem_top);
#endif
}
